...
export function getUserProfile() {
    return new Promise(resolve => {
        setTimeout(() => {
            const profile = JSON.parse(localStorage.getItem('nauticalflow-user-data')) || {
...
export function updateUserProfile(updatedData) {
     return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (updatedData.username === 'user') {
                  reject(new Error('Username already exists.'));
                  return;
            }
            let currentData = JSON.parse(localStorage.getItem('nauticalflow-user-data') || '{}');
            const newData = { ...currentData, ...updatedData, updatedAt: new Date().toISOString() };
            localStorage.setItem('nauticalflow-user-data', JSON.stringify(newData));
            localStorage.setItem('nauticalflow-display-name', newData.displayName);
            resolve(newData);
        }, 1000);
    });
}
